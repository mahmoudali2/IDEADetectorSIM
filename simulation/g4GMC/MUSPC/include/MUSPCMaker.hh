...
#ifndef MUSPCMaker_hh
#define MUSPCMaker_hh
...
public:
  ...
  MUSPCMaker( crd::SimpleConfig const& config );
  ~MUSPCMaker ();

  // This is the accessor that will remain.
  std::unique_ptr<MUSPCtracker> getMUSPCtrackerPtr() { return std::move(_lmst); }
  std::unique_ptr<MUSPCRadiator> getMUSPCradiatorPtr() { return std::move(_lmsrad); }
...
private:
  ...
  int _geomType;                //Verison number (see MUSPCtracker for definifiton) (from it depends the Cell Geometry type: 2 hexagonal, 3 square)
...
//  std::vector<double> _LayersHeight_fwd;
  ...
  std::unique_ptr<MUSPCtracker> _lmst;
  std::unique_ptr<MUSPCRadiator> _lmsrad;
