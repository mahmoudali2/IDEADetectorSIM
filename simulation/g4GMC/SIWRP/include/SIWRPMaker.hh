...
public:
  ...
  SIWRPMaker( crd::SimpleConfig const& config );
  ~SIWRPMaker ();

  // This is the accessor that will remain.
  std::unique_ptr<SIWRPtracker> getSIWRPtrackerPtr() { return std::move(_lpst); }
  //std::unique_ptr<SIWRPRadiator> getSIWRPradiatorPtr() { return std::move(_lpsrad); }
...
private:
  ...
  int _geomType;                //Verison number (see SIWRPtracker for definifiton) (from it depends the Cell Geometry type: 2 hexagonal, 3 square)
...
//  std::vector<double> _LayersHeight_fwd;
  ...
  std::unique_ptr<SIWRPtracker> _lpst;
  //std::unique_ptr<SIWRPRadiator> _lpsrad;
