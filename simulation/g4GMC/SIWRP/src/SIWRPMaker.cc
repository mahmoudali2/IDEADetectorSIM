...
// from arguments.
SIWRPMaker::SIWRPMaker( crd::SimpleConfig const& config):
                                                    _center(){

        _isExternal     = false;
        _extFile        = config.getString("siwrp.extFile");
        if ( _extFile.size()>1 && ( _extFile.find_last_of(".gdml") || _extFile.find_last_of(".GDML") )!=0 ) _isExternal = true;

        _z0             = config.getDouble("siwrp.z0");
        _geomType       = config.getInt("siwrp.geomType");
...
SIWRPMaker::~SIWRPMaker (){}

void SIWRPMaker::loadBarrelTracker( crd::SimpleConfig const& config ){

  _nLayers        = config.getInt("siwrp.Brl.nLayers");

  config.getVectorDouble("siwrp.Brl.LayersInRad", _LayersInnerRad, _nLayers);
  config.getVectorInt("siwrp.Brl.nPhiSectors", _nPhiSectors, _nLayers);
  config.getVectorInt("siwrp.Brl.nLaddersPerSector",_nLaddersPerSector,_nLayers);
  config.getVectorDouble("siwrp.Brl.LaddersHalfLength",_LaddersHalfLength,_nLayers);
  config.getVectorDouble("siwrp.Brl.LaddersThickness",_LaddersThickness,_nLayers);
  config.getVectorInt("siwrp.Brl.ROTypes",_ROTypes,_nLayers);
  config.getVectorDouble("siwrp.Brl.ROfirstSideDim",_ROfirstSideDim,_nLayers);
  config.getVectorDouble("siwrp.Brl.ROfirstSideInsul",_ROfirstSideInsul,_nLayers);
  config.getVectorDouble("siwrp.Brl.ROSecondSideDim",_ROSecondSideDim,_nLayers);
  config.getVectorDouble("siwrp.Brl.ROSecondSideInsul",_ROSecondSideInsul,_nLayers);
  config.getVectorDouble("siwrp.Brl.ROangle",_ROangle,_nLayers);
 ...
 char tmpVarName[50];
  ...
  for (int il=0; il<_nLayers; ++il) {
    ...
    //Ladders Parameters
    sprintf(tmpVarName,"siwrp.Brl.l%d.ld.nShells",il+1);
    const std::string nShlVarName(tmpVarName);
    _LaddersNmShells.push_back( config.getInt(nShlVarName) );

    sprintf(tmpVarName,"siwrp.Brl.l%d.ld.ShellsMaterial",il+1);
    ...
    sprintf(tmpVarName,"siwrp.Brl.l%d.ld.ShellsThickness",il+1);
    ...
    //readOuts Parameters
    sprintf(tmpVarName,"siwrp.Brl.l%d.ro.nShells",il+1);
    const std::string nROShlVarName(tmpVarName);
    _ReadoutsNmShells.push_back( config.getInt(nROShlVarName) );

    sprintf(tmpVarName,"siwrp.Brl.l%d.ro.ShellIds",il+1);
...
//            double FsrSdLadderDim = CLHEP::twopi*(_LayersInnerRad[il]+_LaddersThickness[il])/((double)_nPhiSectors[il]);
      ...
      if (nROFstSdperLad>10000 || nROSndSdperLad>10000) {
        ...
        e<<"SIWRP: Maximum number of Channels allowed per X or Y per Ladder is 10000!\n";
        e.error();
...
void SIWRPMaker::loadForwardTracker( crd::SimpleConfig const& config ){

  _nFwdLayers        = config.getInt("siwrp.Fwd.nLayers",0);

  if (_nFwdLayers==0) {
    std::cout<<"SIWRP: no Forward layers present!"<<std::endl;
    return;
  }
  config.getVectorDouble("siwrp.Fwd.LayersInRad", _LayersInnerRad_fwd, _nFwdLayers);
  config.getVectorDouble("siwrp.Fwd.LayersZPos", _LayersZPos_fwd, _nFwdLayers);
//  config.getVectorDouble("siwrp.Fwd.LayersHeight",_LayersHeight_fwd,_nFwdLayers);
  config.getVectorInt("siwrp.Fwd.nPhiSectors", _nPhiSectors_fwd, _nFwdLayers);
  config.getVectorInt("siwrp.Fwd.nLaddersPerSector",_nLaddersPerSector_fwd,_nFwdLayers);
//  config.getVectorDouble("siwrp.Fwd.LaddersHeight",_LaddersHeight_fwd,_nFwdLayers);
  config.getVectorDouble("siwrp.Fwd.LaddersThickness",_LaddersThickness_fwd,_nFwdLayers);
  config.getVectorInt("siwrp.Fwd.ROTypes",_ROTypes_fwd,_nFwdLayers);
  config.getVectorDouble("siwrp.Fwd.ROfirstSideDim",_ROfirstSideDim_fwd,_nFwdLayers);
  config.getVectorDouble("siwrp.Fwd.ROfirstSideInsul",_ROfirstSideInsul_fwd,_nFwdLayers);
  config.getVectorDouble("siwrp.Fwd.ROSecondSideDim",_ROSecondSideDim_fwd,_nFwdLayers);
  config.getVectorDouble("siwrp.Fwd.ROSecondSideInsul",_ROSecondSideInsul_fwd,_nFwdLayers);
  config.getVectorDouble("siwrp.Fwd.ROangle",_ROangle_fwd,_nFwdLayers);
  ...
  for (int il=0; il<_nFwdLayers; ++il) {
    ...
    sprintf(tmpVarName,"siwrp.Fwd.l%d.ld.LaddersHeight",il+1);
    ...
    //readOuts Parameters
    sprintf(tmpVarName,"siwrp.Fwd.l%d.ro.nShells",il+1);
    const std::string nROShlVarName(tmpVarName);
    _ReadoutsNmShells_fwd.push_back( config.getInt(nROShlVarName) );

    sprintf(tmpVarName,"siwrp.Fwd.l%d.ro.ShellIds",il+1);
    const std::string ROShlIdsVarName(tmpVarName);
    std::vector<int> tmproIds;
...
//            if ( tmpNRwPerAddOneDltRO!=0 && nDlROAdd>0 && nDlROAdd%tmpNRwPerAddOneDltRO==0 ) { ++nROperRow; }
        ...
        if (nROFstSdLngperLad>10000 || nROSndSdperLad>10000) {
          //                throw cet::exception("GEOM") <<"Using GDML file option is temporarily disabled\n";
          exc::exceptionG4 e("GEOM","Fatal Error in Argument",1);
          e<<"SIWRP: Maximum number of Channels allowed per X or Y per Fwd Ladder is 10000!\n";
          e.error();
...
void SIWRPMaker::Build(){

  _lpst = unique_ptr<SIWRPtracker>(new SIWRPtracker());
  _lpst->_isExternal = _isExternal;
  _lpst->_geomType=_geomType;

  if (_nLayers<1) {
    exc::exceptionG4 e("GEOM","Fatal Error in Argument",1);
    e<<"SIWRP: Number of Layer is not acceptable\n";
    e.error();
  }
  ...
  if (_isExternal) {
    //                throw cet::exception("GEOM") <<"Using GDML file option is temporarily disabled\n";
    exc::exceptionG4 e("GEOM","Fatal Error in Argument",1);
    e<<"SIWRP: Using GDML file option is temporarily disabled\n";
    e.error();
    ...
    for ( int ily=0; ily<_nLayers; ++ily ) {
      std::cout <<"Building SIWRP layer: "<<ily+1<<std::endl;
...
//          double ladZpos = 0.0;
    ...
    for (int jly=0; jly<2*_nFwdLayers; ++jly) {
      ...
      std::cout <<"Building SIWRP FW layer: "<<jly+1<<std::endl;
...
/*
void SIWRPMaker::loadBarrelRadiator( crd::SimpleConfig const& config ){

  char tmpVarName[50];

  _nRadiatLayers   = config.getInt("siwrp.Brl.nRadLayers",0);
  for (int il=0; il<_nRadiatLayers; ++il) {
    //Radiator Layers Parameters
    sprintf(tmpVarName,"siwrp.Brl.rad.l%d.InRad",il+1);
    ...
    sprintf(tmpVarName,"siwrp.Brl.rad.l%d.halfLength",il+1);
    ...
    sprintf(tmpVarName,"siwrp.Brl.rad.l%d.nShells",il+1);
    const std::string nShlVarName(tmpVarName);
    _RadiatNmShells.push_back(config.getInt(nShlVarName));

    sprintf(tmpVarName,"siwrp.Brl.rad.l%d.ShellsMaterial",il+1);
    ...
    sprintf(tmpVarName,"siwrp.Brl.rad.l%d.ShellsThickness",il+1);
...
void SIWRPMaker::loadForwardRadiator( crd::SimpleConfig const& config ) {

  char tmpVarName[50];

  _nRadiatLayers_fwd = config.getInt("siwrp.Fwd.nRadLayers",0);
  for (int il=0; il<_nRadiatLayers_fwd; ++il) {
    //Radiator Layers Parameters
    sprintf(tmpVarName,"siwrp.Fwd.rad.l%d.InRad",il+1);
    const std::string inRadVarName(tmpVarName);
    ...
    sprintf(tmpVarName,"siwrp.Fwd.rad.l%d.OutRad",il+1);
...
void SIWRPMaker::BuildRadiator(){

  _lpsrad = unique_ptr<SIWRPRadiator>(new SIWRPRadiator());

  if (_nRadiatLayers<1) {
    exc::exceptionG4 e("GEOM","Warning in Argument",4);
    e<<"SIWRP: Number of Radiator Shell is 0\n";
    e.error();
  }
  if (_nRadiatLayers_fwd<0) {
    exc::exceptionG4 e("GEOM","Warning in Argument",4);
    e<<"SIWRP: Number of Forwad Radiator Shell is negative\n";
    e.error();
  }
